...
Keypad indira = Keypad( makeKeymap(keys), PinBaris,   PinKolom, JmlBaris, JmlKolom ); 
void setup() { 
  ...
  delay(500);
  lcd.clear();
  indira.addEventListener(keypadEvent);
}
void loop() {
  pinMode(led, OUTPUT);
   if (a==0) { //state awal
  ...
  indira.getKey(); //membaca keypad
}
void keypadEvent(KeypadEvent eKey){ 
  switch (indira.getState()){
